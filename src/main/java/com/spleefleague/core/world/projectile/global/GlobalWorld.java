package com.spleefleague.core.world.projectile.global;
    ...
    public static class RotatingItemPlayer {
        ...
        public static class RotatingItem {
            ...
            public RotatingItem(ProjectileWorld<? extends ProjectileWorldPlayer> world, double x, double y, double z, ItemStack item) {
                remainingTime = 40;
                world.getPlayerMap().values().forEach(pwp -> FakeUtil.sendArmorStandSpawn(
                        pwp.getPlayer(),
                        entityId,
                        new Location(Core.OVERWORLD, x, y, z + RADIUS),
                        item));
                this.x = x;
                this.y = y;
                this.z = z + RADIUS;
            ...
            public boolean onTick() {
                if (--remainingTime > 0) {
                    ...
                    world.getPlayerMap().values().forEach(pwp -> FakeUtil.sendEntityDestroy(
                            pwp.getPlayer(),
                            entityId
                    ));
                    return false;
            ...
            public void update(float rotation, short changeX, short changeY, short changeZ) {
                ...
                world.getPlayerMap().values().forEach(pwp -> FakeUtil.sendEntityMoveLook(
                        pwp.getPlayer(),
                        entityId,
                        realChangeX,
                        changeY,
